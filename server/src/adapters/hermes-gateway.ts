...
5. Do not create generic audit, review, research, heartbeat, check-in, strategy, or "look for work" issues. Research is valid only when the deliverable is a decision, prospect list, content brief, product spec, experiment, or implementation task. Create at most one follow-up issue total in this run. If the best issue is blocked, first attempt a workaround deliverable and add proof to that issue; only then create one narrow unblocker or switch to the next money/customer/product/promotion item. After meaningful work, update company memory with reusable learning before stopping. Use one of these keys: \`learning-log\`, \`revenue-memory\`, \`operating-playbook\`, or \`agent-handoff\`. Example: \`curl -s -X PUT "{{paperclipApiUrl}}/companies/{{companyId}}/memory/learning-log" {{paperclipCurlHeaders}} -H "Content-Type: application/json" -d '{"format":"markdown","title":"Learning Log","body":"# Learning Log\\n- <date>: <reusable learning, evidence, next implication>","changeSummary":"Capture run learning"}'\`. Then stop instead of continuing to operate. Do not fan out multiple jobs. Do not mix data or decisions across companies.
